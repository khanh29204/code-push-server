...
pub fn router() -> Router<AppState> {
    Router::new()
        .route("/", get(list_apps))
        .route("/{app_name}/deployments", get(list_deployments).post(create_deployment))
        .route("/{app_name}/deployments/{deployment_name}", get(get_deployment).patch(update_deployment).delete(delete_deployment))
        .route("/{app_name}/deployments/{deployment_name}/metrics", get(get_deployment_metrics))
        .route("/{app_name}/deployments/{deployment_name}/releases/{label}", delete(delete_release_by_label))
        .route("/{app_name}/deployments/{deployment_name}/history", get(get_deployment_history).delete(delete_deployment_history))
        .route("/{app_name}/deployments/{deployment_name}/release", post(release_package).patch(modify_release_package))
        .route("/{app_name}/deployments/{source_deployment_name}/promote/{dest_deployment_name}", post(promote_package))
        .route("/{app_name}/deployments/{deployment_name}/rollback/{label}", post(rollback_with_label))
        .route("/{app_name}/deployments/{deployment_name}/rollback", post(rollback))
        .route("/{app_name}/collaborators", get(get_collaborators))
        .route("/{app_name}/collaborators/{email}", post(add_collaborator).delete(remove_collaborator))
...
async fn list_deployments(
    ...
    let deployments = sqlx::query_as::<_, crate::models::deployments::Deployment>("SELECT * FROM deployments WHERE appid = ?")
        .bind(app.id)
...
async fn get_deployment_metrics(
    ...
    let dep = sqlx::query_as::<_, crate::models::deployments::Deployment>("SELECT * FROM deployments WHERE appid = ? AND name = ?")
        .bind(app.id).bind(&deployment_name).fetch_optional(&state.db.pool).await?.ok_or_else(|| AppError::NotFound("Deployment not found".into()))?;
    let dv = sqlx::query_as::<_, crate::models::deployments_versions::DeploymentVersion>("SELECT * FROM deployments_versions WHERE id = ?")
        .bind(dep.last_deployment_version_id).fetch_optional(&state.db.pool).await?;
    let package_id = dv.map(|v| v.current_package_id).unwrap_or(0);
    let metrics = crate::services::package_manager::PackageManager::get_metrics_by_package_id(&state.db.pool, package_id).await?;
    Ok(Json(MetricsResponse { metrics: serde_json::to_value(metrics).unwrap_or(serde_json::Value::Null) }))
}
...
async fn delete_deployment(
    ...
    let app = crate::services::app_manager::AppManager::find_app_by_name(&state.db.pool, user.id, &app_name)
        .await?.ok_or_else(|| AppError::NotFound(format!("App {} not found", app_name)))?;
    crate::services::deployments_manager::DeploymentsManager::delete_deployment_by_name(&state.db.pool, &deployment_name, app.id).await?;
    Ok(Json(DeploymentResponse { deployment: serde_json::Value::Null }))
}
...
async fn update_deployment(
    ...
    let app = crate::services::app_manager::AppManager::find_app_by_name(&state.db.pool, user.id, &app_name)
        .await?.ok_or_else(|| AppError::NotFound(format!("App {} not found", app_name)))?;
    crate::services::deployments_manager::DeploymentsManager::rename_deployment_by_name(&state.db.pool, &deployment_name, app.id, &payload.name).await?;
    let dep = sqlx::query_as::<_, crate::models::deployments::Deployment>("SELECT * FROM deployments WHERE appid = ? AND name = ?")
        .bind(app.id).bind(&payload.name).fetch_optional(&state.db.pool).await?.ok_or_else(|| AppError::NotFound("Deployment not found".into()))?;
    Ok(Json(DeploymentResponse { deployment: serde_json::json!({ "name": dep.name, "key": dep.deployment_key }) }))
}
...
async fn delete_deployment_history(
    ...
    let dep = sqlx::query_as::<_, crate::models::deployments::Deployment>("SELECT * FROM deployments WHERE appid = ? AND name = ?")
        .bind(app.id).bind(&deployment_name).fetch_optional(&state.db.pool).await?.ok_or_else(|| AppError::NotFound("Deployment not found".into()))?;
    crate::services::deployments_manager::DeploymentsManager::delete_deployment_history(&state.db.pool, dep.id).await?;
    Ok("ok")
}
...
async fn modify_release_package(
    ...
    let dep = sqlx::query_as::<_, crate::models::deployments::Deployment>("SELECT * FROM deployments WHERE appid = ? AND name = ?")
        .bind(app.id).bind(&deployment_name).fetch_optional(&state.db.pool).await?.ok_or_else(|| AppError::NotFound("Deployment not found".into()))?;
    let params: crate::services::package_manager::ReleaseParams = serde_json::from_value(payload.package_info).unwrap_or_default();
    ...
    let package_id = if let Some(label) = &params.label {
        ...
        let dv = sqlx::query_as::<_, crate::models::deployments_versions::DeploymentVersion>("SELECT * FROM deployments_versions WHERE id = ?")
            .bind(dep.last_deployment_version_id).fetch_optional(&state.db.pool).await?.ok_or_else(|| AppError::NotFound("DeploymentVersion not found".into()))?;
...
async fn promote_package(
    ...
    let app = crate::services::app_manager::AppManager::find_app_by_name(&state.db.pool, user.id, &app_name)
        .await?.ok_or_else(|| AppError::NotFound(format!("App {} not found", app_name)))?;
    let source_dep = sqlx::query_as::<_, crate::models::deployments::Deployment>("SELECT * FROM deployments WHERE appid = ? AND name = ?")
        .bind(app.id).bind(&source_deployment_name).fetch_optional(&state.db.pool).await?.ok_or_else(|| AppError::NotFound("Source deployment not found".into()))?;
    let dest_dep = sqlx::query_as::<_, crate::models::deployments::Deployment>("SELECT * FROM deployments WHERE appid = ? AND name = ?")
        .bind(app.id).bind(&dest_deployment_name).fetch_optional(&state.db.pool).await?.ok_or_else(|| AppError::NotFound("Dest deployment not found".into()))?;
        
    let params: crate::services::package_manager::ReleaseParams = serde_json::from_value(payload.package_info).unwrap_or_default();
    let package = crate::services::package_manager::PackageManager::promote_package(&state.db.pool, &source_dep, &dest_dep, params, user.id).await?;
...
async fn handle_rollback(
    ...
    let package = crate::services::package_manager::PackageManager::rollback_package(&state.db.pool, dep.last_deployment_version_id, label.as_deref(), user.id).await?;
...
async fn get_collaborators(
    ...
    let app = crate::services::app_manager::AppManager::find_app_by_name(&state.db.pool, user.id, &app_name)
        .await?.ok_or_else(|| AppError::NotFound(format!("App {} not found", app_name)))?;
    let cols = crate::services::collaborators_manager::CollaboratorsManager::list_collaborators(&state.db.pool, app.id).await?;
    Ok(Json(CollaboratorsResponse { collaborators: serde_json::to_value(cols).unwrap_or(serde_json::Value::Null) }))
}
...
async fn add_collaborator(
    ...
    let app = crate::services::app_manager::AppManager::find_app_by_name(&state.db.pool, user.id, &app_name)
        .await?.ok_or_else(|| AppError::NotFound(format!("App {} not found", app_name)))?;
    let target_user = sqlx::query_as::<_, crate::models::users::User>("SELECT * FROM users WHERE email = ?")
        .bind(&email).fetch_optional(&state.db.pool).await?.ok_or_else(|| AppError::NotFound("User not found".into()))?;
    crate::services::collaborators_manager::CollaboratorsManager::add_collaborator(&state.db.pool, app.id, target_user.id).await?;
    Ok("ok")
}
...
async fn remove_collaborator(
    ...
    let target_user = sqlx::query_as::<_, crate::models::users::User>("SELECT * FROM users WHERE email = ?")
        .bind(&email).fetch_optional(&state.db.pool).await?.ok_or_else(|| AppError::NotFound("User not found".into()))?;
    crate::services::collaborators_manager::CollaboratorsManager::delete_collaborator(&state.db.pool, app.id, target_user.id).await?;
    Ok("ok")
}
...
async fn rename_app(
    ...
    let app = crate::services::app_manager::AppManager::find_app_by_name(&state.db.pool, user.id, &app_name)
        .await?.ok_or_else(|| AppError::NotFound(format!("App {} not found", app_name)))?;
    crate::services::app_manager::AppManager::modify_app(&state.db.pool, app.id, Some(&payload.name), None, None).await?;
    Ok("ok")
}
...
async fn transfer_app(
    ...
    let target_user = sqlx::query_as::<_, crate::models::users::User>("SELECT * FROM users WHERE email = ?")
        .bind(&email).fetch_optional(&state.db.pool).await?.ok_or_else(|| AppError::NotFound("User not found".into()))?;
    crate::services::app_manager::AppManager::transfer_app(&state.db.pool, app.id, user.id, target_user.id).await?;
    Ok("ok")
}
...
async fn create_app(
    ...
    crate::services::app_manager::AppManager::add_app(&state.db.pool, user.id, &payload.name, os, platform, &user.identical).await?;
    let app = crate::services::app_manager::AppManager::find_app_by_name(&state.db.pool, user.id, &payload.name)
        .await?.ok_or_else(|| AppError::NotFound(format!("App {} not found", payload.name)))?;
...
async fn delete_release_by_label(
    ...
    // Check ownership
    crate::services::account_manager::AccountManager::owner_can(&state.db.pool, user.id, &app_name).await?;
