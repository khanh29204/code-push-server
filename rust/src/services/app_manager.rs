...
impl AppManager {
    pub async fn find_app_by_name(
        ...
        let app = sqlx::query_as::<_, App>("SELECT * FROM apps WHERE name = ? AND uid = ?")
            .bind(app_name)
            .bind(uid)
            .fetch_optional(pool)
            .await?;
    ...
    pub async fn delete_app(pool: &SqlitePool, app_id: i64) -> Result<(), AppError> {
        ...
        sqlx::query("DELETE FROM apps WHERE id = ?")
            ...
            .execute(&mut *tx)
            .await?;
        sqlx::query("DELETE FROM collaborators WHERE appid = ?")
            .bind(app_id)
            .execute(&mut *tx)
            .await?;
        sqlx::query("DELETE FROM deployments WHERE appid = ?")
            .bind(app_id)
            .execute(&mut *tx)
            .await?;
        tx.commit().await?;
