...
impl CollaboratorsManager {
    pub async fn list_collaborators(
        pool: &SqlitePool,
        app_id: i64,
    ) -> Result<HashMap<String, CollaboratorPermission>, AppError> {
    ...
    pub async fn add_collaborator(
        ...
        if exists.is_some() {
            return Err(AppError::General(
                "user already is Collaborator.".to_string(),
            ));
        }
    ...
    pub async fn delete_collaborator(
        ...
        let data = sqlx::query_as::<_, Collaborator>(
            "SELECT * FROM collaborators WHERE appid = ? AND uid = ?",
        )
        .bind(app_id)
        .bind(uid)
        .fetch_optional(pool)
